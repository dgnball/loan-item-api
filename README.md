...
| Loan item 123e4567-e89b-12d3-a456-426614174000 to Bob | PUT  |  /loan-items/123e4567-e89b-12d3-a456-426614174000 | ```{"loaned-to": "bob"}```  | "access-token": token  | 
| Get all loan items  | GET  |  /loan-items |   | "access-token": token  | 
| Get all loan items loaned to Bob | GET  |  /loan-items?loaned-to=bob |   | "access-token": token  | 
...
| Get all loan items loaned to Bob where loan-item description contains "boots" | GET  |  /loan-items?loaned-to=bob&description-contains=boots |   | "access-token": token  | 
...
| Delete all loan items where loan-item description contains "drills" | DELETE  |  /loan-items?description-contains=drills  |   | "access-token": token  | 
...
|loan-item| Returned by all calls to /loan-item/:id. Value is a single Loan object. | ```{"loan-item": {"id": "123e4567-e89b-12d3-a456-426614174000", "description": "wheelbarrow","loaned-to": "bob"}}``` |
|loan-items| Returned by all calls to /loan-item (including those with query parameters). Value is an object where the key is the loan-item id and the value is a loan-item object. | ```{"loan-items": {"123e4567-e89b-12d3-a456-426614174000":{"id": "123e4567-e89b-12d3-a456-426614174000", "description": "wheelbarrow","loaned-to": "bob"},"123e4567-e89b-12d3-a456-426614174001":{"id": "123e4567-e89b-12d3-a456-426614174001", "description": "drill","loaned-to": "sally"}}}``` |
...
|users| Returned by all calls to /users. Value is an object where the key is ```username``` mapping to a user object. | ```{'users': {'admin': {'phone': "+441234567890", 'role': 3, 'username': 'admin'}, 'bob': {'phone': "+441234567890", 'role': 1, 'username': 'bob'}}}```|
